...
function YouVideo() {
  ...
  const search = (query) => {
    ...
    fetch(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=30&q=${query}&type=video&key=AIzaSyAsS5JIMXgCYmI67K-wuuyxUcbDwGJ0p8s`, requestOptions)
  ...
  useEffect(()=>{
    ...
    fetch("https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=30&q=webstoryboy&type=video&key=AIzaSyAsS5JIMXgCYmI67K-wuuyxUcbDwGJ0p8s", requestOptions)
      .then(response => response.json())
      .then(result => setVideos(result.items))
      .catch(error => console.log('error', error));
  },[]);
